...
<div class="uk-container uk-margin-small-top">
    <div uk-filter="target: .js-filter">
        ...
        <ul class="uk-margin-medium-bottom js-filter uk-grid-small uk-child-width-1-2 uk-child-width-1-3@m uk-text-center uk-padding-small uk-border-rounded" uk-grid style="background-color: #fafafa;">
            ...
            {% comment %} 1. CAMBIO: Usar site.sacos y ordenar por el campo 'lado' (según tu imagen) {% endcomment %}
            {% assign sacos_ordenados = site.sacos | sort: "lado" %}
            
            {% for saco in sacos_ordenados %}
            {% comment %} 2. CAMBIO: Usar 'categorías' (con tilde) como está en tu Front Matter {% endcomment %}
            <li class="{{ saco.categorías | downcase }}">
                <div class="uk-card uk-card-default uk-card-hover uk-border-rounded uk-overflow-hidden card-sombra-soft uk-transition-toggle">
                    ...
                    <div class="uk-card-media-top uk-inline uk-overflow-hidden">
                        {% if saco.categorías == "Especial" %}
                        <div style="position: absolute; top: 12px; right: 12px; z-index: 10;">
                            <span uk-icon="icon: star; ratio: 0.9" style="color: #D4AF37;"></span>
                        </div>
                        {% endif %}

                        {% comment %} 3. CAMBIO: Usar saco.url y saco.imagen {% endcomment %}
                        <a href="{{ saco.url | relative_url }}">
                            <img src="{{ saco.imagen | relative_url }}" alt="{{ saco.título }}" 
